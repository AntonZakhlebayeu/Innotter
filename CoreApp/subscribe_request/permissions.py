...
class IsBlockedPageCreate(BasePermission):
    def has_permission(self, request, view):
        ...
        return datetime.now().replace(tzinfo=pytz.timezone('US/Eastern')) > \
...
class IsBlockedPageUpdate(BasePermission):
    def has_permission(self, request, view):
        if SubscribeRequest.objects.get(pk=view.kwargs['pk']).desired_page.unblock_date is None:
            return True
...
class IsOwner(BasePermission):
    def has_permission(self, request, view):
        return request.user.pk == SubscribeRequest.objects.get(pk=view.kwargs['pk']).desired_page.owner.pk


class IsOwnerToAcceptAllSubscribeRequests(BasePermission):
    def has_permission(self, request, view):
        return request.user.pk == Page.objects.get(pk=request.data.get('desired_page')).owner.pk
